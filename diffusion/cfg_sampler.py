...
class ClassifierFreeSampleModel(nn.Module):
    ...
    def forward(self, batch, timesteps):
        ...
        out = self.model(batch, timesteps, force_mask=False)
        out_uncond = self.model(batch, timesteps, force_mask=True)
        # return out_uncond + (y['scale'].view(-1, 1, 1, 1) * (out - out_uncond))   # todo: what does it mean?
        return out_uncond + self.guidance_param * (out - out_uncond)
